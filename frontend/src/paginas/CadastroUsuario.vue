...
import { usuarioCadastroSchema } from '@shared/schemas/usuario.schema.js';
...
const { defineField, handleSubmit, errors } = useForm({
  validationSchema: toTypedSchema(usuarioCadastroSchema),
...
const aoEnviar = handleSubmit(async (valores) => {
  ...
  enviando.value = true;
  try {
    await authApi.cadastrar(valores);
    const usuario = await authApi.login({ email: valores.email, senha: valores.senha });
    sessao.definirUsuario(usuario);
    router.push('/produtos');
  } catch (erro) {
    erroGeral.value = erro instanceof ErroApi ? erro.message : 'Não foi possível concluir o cadastro.';
  } finally {
...
<template>
  <div class="pagina-auth">
    <div class="pagina-auth__card">
      ...
      <h1 class="pagina-auth__titulo">Criar conta</h1>
      <p class="pagina-auth__subtitulo">Cadastre-se para acessar o catálogo de produtos.</p>
      ...
      <form class="pagina-auth__form" novalidate @submit="aoEnviar">
        <Campo id="nome" v-model="nome" rotulo="Nome" obrigatorio :erro="errors.nome" placeholder="Seu nome completo" />
        ...
        <Botao tipo="submit" :carregando="enviando" style="width: 100%; margin-top: 8px;">Criar conta</Botao>
      </form>
